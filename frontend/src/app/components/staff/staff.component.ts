...
import { StaffEditModalComponent } from './modals/staff-edit-modal';
...
export class StaffComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['first_name', 'last_name', 'address_id', 'picture', 'email', 'store_id', 'active', 'username', 'last_update', 'delete-instance'];
  ...
  ngOnInit() {
    ...
    this.picture.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['picture.eq'] = this.picture.value;
        this.getData();
      });
    ...
    this.username.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['username.eq'] = this.username.value;
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.staffGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.staff_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('staff successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('staff item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.staffDelete(ids).subscribe(res => {
