...
import { Film_actorEditModalComponent } from './modals/film_actor-edit-modal';

/*
 * "Datagrid" component for displaying instance of film_actor
 * entities from your HTTP REST backend.
 */
@Component({
  selector: 'app-film_actor',
  templateUrl: './film_actor.component.html',
  styleUrls: ['./film_actor.component.scss']
})
export class Film_actorComponent implements OnInit {

  // Actual data currently displayed in the grid. The mat-table will be databound to this list.
  private data: any[];

  // Which columns we should display. Reorder to prioritize columns differently.
  private displayedColumns: string[] = ['actor_id', 'film_id', 'last_update', 'delete-instance'];
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.film_actorGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.film_actor_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(Film_actorEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('film_actor successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('film_actor item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.film_actorDelete(ids).subscribe(res => {
