...
import { PaymentEditModalComponent } from './modals/payment-edit-modal';
...
export class PaymentComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['customer_id', 'staff_id', 'rental_id', 'amount', 'payment_date', 'last_update', 'delete-instance'];
  ...
  ngOnInit() {
    ...
    this.payment_id = new FormControl('');
    this.payment_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['payment_id.eq'] = this.payment_id.value;
        this.getData();
      });
    ...
    this.amount.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['amount.eq'] = this.amount.value;
        this.getData();
      });
    this.payment_date = new FormControl('');
    this.payment_date.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['payment_date.eq'] = this.payment_date.value;
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.paymentGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.payment_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(PaymentEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('payment successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('payment item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.paymentDelete(ids).subscribe(res => {
