...
import { Film_textEditModalComponent } from './modals/film_text-edit-modal';

/*
 * "Datagrid" component for displaying instance of film_text
 * entities from your HTTP REST backend.
 */
@Component({
  selector: 'app-film_text',
  templateUrl: './film_text.component.html',
  styleUrls: ['./film_text.component.scss']
})
export class Film_textComponent implements OnInit {

  // Actual data currently displayed in the grid. The mat-table will be databound to this list.
  private data: any[];

  // Which columns we should display. Reorder to prioritize columns differently.
  private displayedColumns: string[] = ['film_id', 'title', 'description', 'delete-instance'];
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.film_textGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.film_text_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(Film_textEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('film_text successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('film_text item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.film_textDelete(ids).subscribe(res => {
