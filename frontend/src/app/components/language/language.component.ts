...
import { LanguageEditModalComponent } from './modals/language-edit-modal';
...
export class LanguageComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['name', 'last_update', 'delete-instance'];
  ...
  ngOnInit() {
    ...
    this.name.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['name.eq'] = this.name.value;
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.languageGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.language_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(LanguageEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('language successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('language item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.languageDelete(ids).subscribe(res => {
