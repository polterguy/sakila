...
import { ActorEditModalComponent } from './modals/actor-edit-modal';
...
export class ActorComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['first_name', 'last_name', 'last_update', 'delete-instance'];
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.actorGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.actor_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(ActorEditModalComponent, {
      data
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('actor item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.actorDelete(ids).subscribe(res => {
