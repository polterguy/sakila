...
import { RentalEditModalComponent } from './modals/rental-edit-modal';
...
export class RentalComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['rental_date', 'inventory_id', 'customer_id', 'return_date', 'staff_id', 'last_update', 'delete-instance'];
  ...
  private rental_id: FormControl;
  private rental_date: FormControl;
  private inventory_id: FormControl;
  private customer_id: FormControl;
  private return_date: FormControl;
  private staff_id: FormControl;
  ...
  ngOnInit() {
    ...
    this.rental_id = new FormControl('');
    this.rental_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['rental_id.eq'] = this.rental_id.value;
        this.getData();
      });
    this.rental_date = new FormControl('');
    this.rental_date.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['rental_date.eq'] = this.rental_date.value;
        this.getData();
      });
    this.inventory_id = new FormControl('');
    this.inventory_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['inventory_id.eq'] = this.inventory_id.value;
        this.getData();
      });
    ...
    this.customer_id.valueChanges
      ...
      .subscribe(query => {
        ...
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['customer_id.eq'] = this.customer_id.value;
        this.getData();
      });
    this.return_date = new FormControl('');
    this.return_date.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['return_date.eq'] = this.return_date.value;
        this.getData();
      });
    this.staff_id = new FormControl('');
    this.staff_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['staff_id.eq'] = this.staff_id.value;
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.rentalGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.rental_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(RentalEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('rental successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('rental item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.rentalDelete(ids).subscribe(res => {
