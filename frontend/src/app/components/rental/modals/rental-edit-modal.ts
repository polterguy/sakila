 ...
 */
@Component({
  templateUrl: './rental-edit-modal.html',
  styleUrls: ['./rental-edit-modal.scss']
})
export class RentalEditModalComponent {
   ...
   */
  private createColumns: string[] = ['rental_date', 'inventory_id', 'customer_id', 'return_date', 'staff_id', 'last_update'];
  private updateColumns: string[] = ['rental_id', 'rental_date', 'inventory_id', 'customer_id', 'return_date', 'staff_id', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<RentalEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.rentalPut(this.data.entity).subscribe(res => {
      ...
      this.service.rentalPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
