...
import { Film_categoryEditModalComponent } from './modals/film_category-edit-modal';

/*
 * "Datagrid" component for displaying instance of film_category
 * entities from your HTTP REST backend.
 */
@Component({
  selector: 'app-film_category',
  templateUrl: './film_category.component.html',
  styleUrls: ['./film_category.component.scss']
})
export class Film_categoryComponent implements OnInit {

  // Actual data currently displayed in the grid. The mat-table will be databound to this list.
  private data: any[];

  // Which columns we should display. Reorder to prioritize columns differently.
  private displayedColumns: string[] = ['film_id', 'category_id', 'last_update', 'delete-instance'];
  ...
  private category_id: FormControl;
  ...
  ngOnInit() {
    ...
    this.category_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['category_id.eq'] = this.category_id.value;
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.film_categoryGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.film_category_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(Film_categoryEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('film_category successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('film_category item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.film_categoryDelete(ids).subscribe(res => {
