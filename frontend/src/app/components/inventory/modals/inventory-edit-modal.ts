...
@Component({
  ...
  styleUrls: ['./inventory-edit-modal.scss']
})
export class InventoryEditModalComponent {
   ...
   */
  private createColumns: string[] = ['film_id', 'store_id', 'last_update'];
  private updateColumns: string[] = ['inventory_id', 'film_id', 'store_id', 'last_update'];
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.inventoryPut(this.data.entity).subscribe(res => {
      ...
      this.service.inventoryPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
