 ...
 */
@Component({
  templateUrl: './film_actor-edit-modal.html',
  styleUrls: ['./film_actor-edit-modal.scss']
})
export class Film_actorEditModalComponent {
   ...
   */
  private createColumns: string[] = ['actor_id', 'film_id', 'last_update'];
  private updateColumns: string[] = ['actor_id', 'film_id', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<Film_actorEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.film_actorPut(this.data.entity).subscribe(res => {
      ...
      this.service.film_actorPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
