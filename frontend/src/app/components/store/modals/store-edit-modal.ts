...
@Component({
  ...
  styleUrls: ['./store-edit-modal.scss']
})
export class StoreEditModalComponent {
   ...
   */
  private createColumns: string[] = ['manager_staff_id', 'address_id', 'last_update'];
  private updateColumns: string[] = ['store_id', 'manager_staff_id', 'address_id', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<StoreEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.storePut(this.data.entity).subscribe(res => {
      ...
      this.service.storePost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
