...
import { StoreEditModalComponent } from './modals/store-edit-modal';

/*
 * "Datagrid" component for displaying instance of store
...
export class StoreComponent implements OnInit {
  ...
  private displayedColumns: string[] = ['manager_staff_id', 'address_id', 'last_update', 'delete-instance'];
  ...
  private manager_staff_id: FormControl;
  ...
  ngOnInit() {
    ...
    this.manager_staff_id = new FormControl('');
    this.manager_staff_id.valueChanges
      .pipe(debounceTime(this.debounce), distinctUntilChanged())
      .subscribe(query => {
        this.paginator.pageIndex = 0;
        this.filter.offset = 0;
        this.hasFiltered = true;
        this.filter['manager_staff_id.eq'] = this.manager_staff_id.value;
        this.getData();
      });
  ...
  getData(countRecords: boolean = true) {
    ...
    this.httpService.storeGet(this.filter).subscribe(res => {
      this.data = res;
      ...
      if (countRecords) {
        ...
        this.httpService.store_countGet(cloned).subscribe(res2 => {
  ...
  editDetails(entity: any) {
    ...
    const dialogRef = this.dialog.open(StoreEditModalComponent, {
      data
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.snackBar.open('store successfully updated', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  createNewRecord() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res !== null && res !== undefined) {
        ...
        this.getData();
        this.snackBar.open('store item successfully created', 'Close', {
          duration: 2000,
        });
      }
    });
  ...
  delete(entity: any, ids: any) {
    ...
    this.httpService.storeDelete(ids).subscribe(res => {
