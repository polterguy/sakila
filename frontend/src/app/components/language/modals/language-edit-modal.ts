 ...
 */
@Component({
  templateUrl: './language-edit-modal.html',
  styleUrls: ['./language-edit-modal.scss']
})
export class LanguageEditModalComponent {
   ...
   */
  private createColumns: string[] = ['name', 'last_update'];
  private updateColumns: string[] = ['language_id', 'name', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<LanguageEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.languagePut(this.data.entity).subscribe(res => {
      ...
      this.service.languagePost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
