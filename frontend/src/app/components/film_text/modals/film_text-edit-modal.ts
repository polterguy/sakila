...
/*
 * Modal dialog for editing your existing film_text entity types, and/or
 * creating new entity types.
 */
@Component({
  templateUrl: './film_text-edit-modal.html',
  styleUrls: ['./film_text-edit-modal.scss']
})
export class Film_textEditModalComponent {
   ...
   */
  private createColumns: string[] = ['film_id', 'title', 'description'];
  private updateColumns: string[] = ['film_id', 'title', 'description'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<Film_textEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.film_textPut(this.data.entity).subscribe(res => {
      ...
      this.service.film_textPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
