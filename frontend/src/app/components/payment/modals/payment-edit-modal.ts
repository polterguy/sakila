 ...
 */
@Component({
  templateUrl: './payment-edit-modal.html',
  styleUrls: ['./payment-edit-modal.scss']
})
export class PaymentEditModalComponent {
   ...
   */
  private createColumns: string[] = ['customer_id', 'staff_id', 'rental_id', 'amount', 'payment_date', 'last_update'];
  private updateColumns: string[] = ['payment_id', 'customer_id', 'staff_id', 'rental_id', 'amount', 'payment_date', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<PaymentEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.paymentPut(this.data.entity).subscribe(res => {
      ...
      this.service.paymentPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
