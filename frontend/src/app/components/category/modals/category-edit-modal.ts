...
@Component({
  ...
  styleUrls: ['./category-edit-modal.scss']
})
export class CategoryEditModalComponent {
  ...
  private updateColumns: string[] = ['category_id', 'name', 'last_update'];
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.categoryPut(this.data.entity).subscribe(res => {
      ...
      this.service.categoryPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
