...
@Component({
  ...
  styleUrls: ['./country-edit-modal.scss']
})
export class CountryEditModalComponent {
   ...
   */
  private createColumns: string[] = ['country', 'last_update'];
  private updateColumns: string[] = ['country_id', 'country', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<CountryEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.countryPut(this.data.entity).subscribe(res => {
      ...
      this.service.countryPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
