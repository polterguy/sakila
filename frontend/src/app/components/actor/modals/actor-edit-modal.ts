 ...
 */
@Component({
  templateUrl: './actor-edit-modal.html',
  styleUrls: ['./actor-edit-modal.scss']
})
export class ActorEditModalComponent {
   ...
   */
  private createColumns: string[] = ['first_name', 'last_name', 'last_update'];
  private updateColumns: string[] = ['actor_id', 'first_name', 'last_name', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<ActorEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.actorPut(this.data.entity).subscribe(res => {
      ...
      this.service.actorPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
