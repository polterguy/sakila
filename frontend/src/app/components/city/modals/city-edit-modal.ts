...
@Component({
  ...
  styleUrls: ['./city-edit-modal.scss']
})
export class CityEditModalComponent {
   ...
   */
  private createColumns: string[] = ['city', 'country_id', 'last_update'];
  private updateColumns: string[] = ['city_id', 'city', 'country_id', 'last_update'];

  /*
   * Constructor taking a bunch of services injected using dependency injection.
   */
  constructor(
    public dialogRef: MatDialogRef<CityEditModalComponent>,
  ...
  save() {
    ...
    if (this.data.isEdit) {
      ...
      this.service.cityPut(this.data.entity).subscribe(res => {
      ...
      this.service.cityPost(this.data.entity).subscribe(res => {
        this.dialogRef.close(this.data.entity);
